...
import { MoreActionsContext } from "@/context/MoreActionsContext";

const MoreActionsMenu = ({ fetchFiles }) => {
  const { isOpen, closeDropdown } = useContext(MoreActionsContext);
      ...
      {dropdownItems.map((item, idx) =>
        item.type === "divider" ? (
          ...
          <DropdownItem
            key={idx}
            icon={item.icon}
            title={uploading && item.id === "file-upload" ? "Uploading..." : item.title}
            shortcut={item.shortcut}
            arrow={item.arrow}
            onClick={item.id === "file-upload" ? triggerFileInput : item.onClick}
